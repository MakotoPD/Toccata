...
On Windows that produces an installer under
`target/release/bundle/nsis/`. Installing it is all a user has to do — the
FFmpeg libraries travel inside the installer, and the WebView2 runtime is
...
- **Linux** — reading a drive usually needs membership of the `cdrom` or
  ...
  group.
- **macOS** — the system mounts audio CDs by itself and holds on to the device.
  Unmount the disc with `diskutil unmountDisk` before ripping. Apple Silicon
  machines have no built-in drive, so an external USB one is the only option.
